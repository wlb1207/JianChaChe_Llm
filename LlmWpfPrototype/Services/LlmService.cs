...

{
    ...
    private const string StreamingOnlyModel = "gpt-5.4";
    ...
    private async Task<(bool Succeeded, string? NormalizedJson, int ResponseChars)> TryRepairJsonResponseAsync(
        string originalUserInput,
        LlmChatRequestOptions? requestOptions,
        string apiKey,
        string requestId,
        string originalAssistantContent,
        CancellationToken cancellationToken)
    {
        try
        {
            var repairPrompt = BuildJsonRepairPrompt(originalUserInput, originalAssistantContent);
            var repairResponseText = await SendChatCompletionAsync(
                repairPrompt,
            ...
            var responseChars = repairResponseText.Length;

            try
            {
                var normalized = NormalizeModelResponseWithDiagnostics(repairResponseText, originalUserInput, requestOptions);
                return (true, normalized, responseChars);
            }
            catch (LlmResponseParseException)
            {
                return (false, null, responseChars);
            }
        }
        catch
        {
    ...
    private string BuildJsonRepairPrompt(string originalUserInput, string originalAssistantContent)
    {
        var schemaJson = JsonSerializer.Serialize(CreateJsonSchema(), CompactJsonOptions);
        return $"""
You must repair the previous assistant output into a valid JSON object that deserializes to LlmParseResult.
Original user input:
{originalUserInput}
Original assistant output:
{originalAssistantContent}
Current supported JSON schema:
{schemaJson}
Current supported actions/commands:
- query_editable_parameters
- reset_model_workspace
- scan_solidworks_dimensions
- recommend_truss_parameter_mapping
- confirm_truss_parameter_mapping
- open_working_model
- update_solidworks_dimensions
Requirements:
- Output exactly one JSON object.
- Do not output Markdown, code fences, or explanatory text.
- The object must include reply, message, assistantText, actions, commands, parameters, needConfirmation, questions.
- For editable-parameter questions, use query_editable_parameters.
- If no action is needed, return actions=[], commands=[], parameters=[].
""";
    }
...

    {
...

        {
            ...
            var actionJsonContract = requestOptions?.ReplyOnlyMode != true;
            EmitDiagnostic($"[LLM Chat] ActionJsonContract={actionJsonContract}");
            EmitDiagnostic($"[LLM Chat] ActionJsonContractReason={(actionJsonContract ? "ActionGenerationRequired" : "ReplyOnlyMode")}");
            ...
            catch (LlmResponseParseException) when (requestOptions?.ReplyOnlyMode != true)
            {
                EmitDiagnostic("[LLM Chat] JsonRepairRetry=True");
                EmitDiagnostic("[LLM Chat] JsonRepairRetryReason=NonReplyOnlyParseFailure");

                var repairResult = await TryRepairJsonResponseAsync(
                    userInput,
                    requestOptions,
                    apiKey,
                    requestId,
                ...
                EmitDiagnostic($"[LLM Chat] JsonRepairRetrySucceeded={repairResult.Succeeded}");
                EmitDiagnostic($"[LLM Chat] JsonRepairRetryResponseChars={repairResult.ResponseChars}");

                if (repairResult.Succeeded && !string.IsNullOrWhiteSpace(repairResult.NormalizedJson))
                {
                    return repairResult.NormalizedJson;
                }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var requestUrl = BuildChatCompletionsUrl(_options.ApiBaseUrl);
        var requestPath = TryGetRequestPath(requestUrl);
        var shouldUseStreaming = ShouldUseStreaming(requestPath);
        var attempts = shouldUseStreaming
            ? [ResponseFormatMode.None]
            : new[]
            {
                responseFormatMode,
        ...
        {
            EmitDiagnostic("[LLM Chat] StructuredResponseFormatSkipped=True");
            EmitDiagnostic("[LLM Chat] StructuredResponseFormatSkipReason=StreamingCompatibility");
        }
        ...
        foreach (var attempt in attempts)
        {
            try
            {
                EmitDiagnostic($"[LLM Chat] ResponseFormatAttempt={attempt}");
                var payload = BuildPayload(userInput, conversationHistory, attempt, systemPrompt, shouldUseStreaming);
                WriteRequestDiagnostics(
                    purpose: "LLM Chat",
                    requestUrl,
                    requestPath,
                ...
                using var response = await _httpClient.SendAsync(
                    ...
                    shouldUseStreaming ? HttpCompletionOption.ResponseHeadersRead : HttpCompletionOption.ResponseContentRead,
...

                {
                    ...
                    var errorMetadata = TryExtractResponseMetadata(errorResponse);

                    WriteResponseDiagnostics("LLM Chat", response.StatusCode, errorMetadata, errorResponse, errorMetadata.AssistantContentPreview);
                    ...
                    WriteErrorDiagnostics(
                        ...
                        errorResponsePreview: Truncate(errorResponse, RawErrorPreviewLimit),

                        exception: null);



                    var errorMessage = $"LLM ???????????TTP {(int)response.StatusCode}??????{errorResponse}";

                    if (ShouldRetryWithoutStructuredFormat(response.StatusCode, errorResponse, attempt))
...

                {
                    ...
                    EmitDiagnostic("[LLM Chat] StreamingResponse=True");

                    EmitDiagnostic("[LLM Chat] StreamReason=ModelRequiresStream");

                    var streamingResult = await ReadStreamingChatCompletionAsync(response, cancellationToken);

                    WriteResponseDiagnostics("LLM Chat", response.StatusCode, streamingResult.Metadata, streamingResult.RawPreview, Truncate(streamingResult.Content, AssistantPreviewLimit));



                    if (string.IsNullOrWhiteSpace(streamingResult.Content))

                    {

                        throw new InvalidOperationException("gpt-5.4 streaming response produced empty assistant content.");

                    }



                    return streamingResult.Content;
...

    {
        ...
        catch (LlmResponseParseException ex)
        {
            ...
            {
                EmitDiagnostic("[LLM Chat] PlainTextReplyWrapped=True");
                EmitDiagnostic("[LLM Chat] PlainTextReplyWrapReason=ReplyOnlyParseFallback");
                EmitDiagnostic("[LLM Chat] WrappedActions.Count=0");
                EmitDiagnostic("[LLM Chat] WrappedCommands.Count=0");
                EmitDiagnostic("[LLM Chat] ParsedActionCount=0");
                return BuildPlainTextReplyParseResultJson(content);
            }

            EmitDiagnostic("[LLM Chat] PlainTextReplyWrapped=False");
            EmitDiagnostic("[LLM Chat] PlainTextReplyWrapSkippedReason=ActionGenerationMayBeRequired");
            EmitDiagnostic("[LLM Chat] ParsedActionCount=0");
    ...
    private string BuildPlainTextReplyParseResultJson(string assistantContent)
    {
        var trimmed = assistantContent.Trim();
        var parseResult = new LlmParseResult
        {
            Reply = trimmed,
            Message = trimmed,
    ...
    private bool ShouldUseStreaming(string requestPath)

    {

        return string.Equals(_options.Provider, OpenAiProvider, StringComparison.OrdinalIgnoreCase) &&

               string.Equals(requestPath, ChatCompletionsPath, StringComparison.OrdinalIgnoreCase) &&

               string.Equals(_options.Model, StreamingOnlyModel, StringComparison.Ordinal);
    ...
    private async Task<StreamingChatCompletionResult> ReadStreamingChatCompletionAsync(
...

    {
        ...
        var rawPreviewBuilder = new StringBuilder();
...

        {
            ...
            if (rawPreviewBuilder.Length < RawResponsePreviewLimit)

            {

                rawPreviewBuilder.AppendLine(payload);
...

            {
                ...
                metadata = MergeStreamingMetadata(metadata, document.RootElement);



                var chunkContent = TryExtractStreamingAssistantContent(document.RootElement);
                ...
                aggregateBuilder.Append(chunkContent);
...

            {
                ...
                EmitDiagnostic("[LLM Chat] StreamChunkParseError=True");

                EmitDiagnostic($"[LLM Chat] StreamChunkParseErrorPreview=<redacted>, Length={payload.Length}");

                EmitDiagnostic($"[LLM Chat] StreamChunkParseErrorMessage={ex.Message}");
        ...
        var content = aggregateBuilder.ToString();

        EmitDiagnostic($"[LLM Chat] StreamChunkCount={chunkCount}");

        EmitDiagnostic($"[LLM Chat] StreamAggregatedContentChars={content.Length}");
        ...
        return new StreamingChatCompletionResult(
            ...
            metadata with { AssistantContentPreview = Truncate(content, AssistantPreviewLimit) },

            Truncate(rawPreviewBuilder.ToString(), RawResponsePreviewLimit));

    }



    private static LlmResponseMetadata MergeStreamingMetadata(LlmResponseMetadata current, JsonElement root)

    {
        ...
        return new LlmResponseMetadata(
            ...
            TryGetString(root, "id") ?? current.ResponseId,

            TryGetString(root, "object") ?? current.ResponseObject,

            TryGetString(root, "model") ?? current.ResponseModel,

            TryGetInt32(usage, "prompt_tokens") ?? current.UsagePromptTokens,

            TryGetInt32(usage, "completion_tokens") ?? current.UsageCompletionTokens,

            TryGetInt32(usage, "total_tokens") ?? current.UsageTotalTokens,

            choice.ValueKind == JsonValueKind.Object ? TryGetString(choice, "finish_reason") ?? current.FinishReason : current.FinishReason,

            current.AssistantContentPreview);

    }



    private static string TryExtractStreamingAssistantContent(JsonElement root)
...
24. For those general capability questions, reply concisely that the current supported modification is configured truss member section dimensions. If possible, mention configured groups such as truss upper chord section size and truss lower chord section size, with abstract examples:
...
28. A good greeting/help reply is: 你好，我可以帮你查看模型、查询可编辑参数，或者修改已配置的尺寸。
...
Action-generation contract:
- Return exactly one JSON object.
- Do not output Markdown, code fences, or explanatory text.
- The JSON must deserialize to LlmParseResult.
- Top-level fields must include reply, actions, commands, parameters, needConfirmation, and questions, even when empty.
- Use `query_editable_parameters` for editable-parameter questions such as:
  - 查询可编辑参数
  - 有哪些参数可以改
  - 我能修改哪些杆件尺寸
  - 可编辑参数列表
- For execution, query, scan, open, modify, reset, recommend, and confirm requests, return the matching actions/commands instead of plain natural language.
""";
...
""";
    ...
    private sealed record StreamingChatCompletionResult(
